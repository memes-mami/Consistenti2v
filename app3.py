...
import argparse
from fvcore.nn import FlopCountAnalysis  # Import FlopCountAnalysis from fvcore
...
class AnimateController:
    ...
    def animate(self, prompt, negative_prompt, input_image_path, sample_steps=100, width=256, height=256,  
        ...
        img_transform = T.Compose([T.ToTensor(), T.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])])
        ...
        # Measure FLOPs using FlopCountAnalysis before the forward pass
        flop_analysis = FlopCountAnalysis(self.pipeline.unet, (first_frame,))  # Analyze based on first frame input
        print(f"FLOPs for the model: {flop_analysis.total()}")

        # Generate animation
        start_time = time.time()
        sample = self.pipeline(**inputs).videos
        end_time = time.time()

        # Measure time for performance analysis
        duration = end_time - start_time
        throughput = 16 / duration  # Assuming 16 frames per video
        ...
        sample_config = {
            ...
            "height": height,
            "seed": seed,
            "flops": flop_analysis.total(),  # Log the FLOPs in the config
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample_steps", type=int, default=100, help="Number of sampling steps.")  
    parser.add_argument("--width", type=int, default=256, help="Width of the generated video.")     
    parser.add_argument("--height", type=int, default=256, help="Height of the generated video.")   
    ...
    parser.add_argument("--center_crop", action="store_true", help="Apply center cropping to the input image.")  
