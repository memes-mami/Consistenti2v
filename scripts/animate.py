...
torch.cuda.set_device(device)  # Forcefully set CUDA device if available
...
def main(args, config):
    ...
    if config.pipeline_pretrained_path is None:
        ...
        vae = AutoencoderKL.from_pretrained(config.pretrained_model_path, subfolder="vae", use_safetensors=True).to(device)
    ...
    for prompt_idx, (prompt, n_prompt, first_frame_path, random_seed) in enumerate(zip(prompts, n_prompts, first_frame_paths, random_seeds)):
        ...
        ).videos.to(device)  # Ensure sample is on CUDA device
    ...
    samples = torch.concat(samples).to(device)  # Ensure samples are moved to CUDA
